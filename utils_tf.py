...
def ReadExpoTimes(fileName):
    return np.power(2.0, np.loadtxt(fileName) + 2.0)
...
def ReadLabel_ICCP(fileName):
    #label = imageio.imread(os.path.join(fileName, 'HDRImg.hdr'), 'hdr')
    #label = label[:, :, [2, 1, 0]]  ##cv2
    label = cv2.imread(os.path.join(fileName, 'hdr_gt.hdr'), -1)
    label = label.astype(np.float32)
    return label
...
def ReadLabel_Prabahkar(fileName):
    #label = imageio.imread(os.path.join(fileName, 'HDRImg.hdr'), 'hdr')
    #label = label[:, :, [2, 1, 0]]  ##cv2
    label = cv2.imread(os.path.join(fileName, 'gt.hdr'), -1)
    label = label.astype(np.float32)
    return label
